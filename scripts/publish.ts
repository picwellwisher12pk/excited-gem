...
async function verifyDraftVersion(
  ...
  console.log(
    `📋 Draft version on CWS: ${draftVersion} (expected: ${expectedVersion})`
  )
...
async function run() {
  try {
    ...
    console.log(
      `🎉 v${expectedVersion} successfully uploaded and submitted to Chrome Web Store!`
    )
  } catch (error) {
