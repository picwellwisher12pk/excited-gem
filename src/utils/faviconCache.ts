...
export class FaviconCache {
    ...
    public extractDomain(url: string): string | null {
        try {
            ...
            if (parsed.protocol.startsWith('chrome') || parsed.protocol.startsWith('edge') || parsed.protocol.startsWith('about')) {
    ...
    public getOrSet(url: string, faviconUrl?: string): string {
        ...
        const domain = this.extractDomain(url)
        const finalFavicon = faviconUrl || (domain && !domain.startsWith('chrome') ? `https://www.google.com/s2/favicons?domain=${domain}&sz=32` : '')
