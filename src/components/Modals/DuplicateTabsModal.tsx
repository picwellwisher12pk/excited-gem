import { Button, Checkbox, List, Modal, Typography, Space, Tooltip, Progress } from 'antd'
...
export const DuplicateTabsModal: React.FC<DuplicateTabsModalProps> = ({ visible, onClose }) => {
    ...
    return (
        <Modal
            title={
                <Space>
                    <Title level={4} style={{ margin: 0 }}>Duplicate Tabs</Title>
            ...
            footer={
                <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                    ...
                    <div className="flex justify-end gap-2 w-full sm:w-auto">
                        ...
                        <Button key="keep-one" onClick={handleSelectAllDuplicates} disabled={isProcessing}>
                            Select Duplicates (Keep 1st)
                        </Button>
                        <Button key="select-all" onClick={handleSelectAll} disabled={isProcessing}>
                            Select All
                        </Button>
                        <Button
                            key="remove"
                    ...
                    <List
                        ...
                        renderItem={({ url, tabs: groupTabs }) => (
                            <List.Item className="border-b border-gray-100 py-4">
                                <div className="mb-2 font-medium text-blue-600 truncate" title={url}>
                                    {url}
                                </div>
                                <div className="pl-4 space-y-2">
                                    {groupTabs.map((tab: any) => (
                                        <div key={tab.id} className="flex items-center justify-between bg-slate-50 p-2 rounded hover:bg-slate-100 transition-colors">
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <span className="truncate text-sm text-gray-700" title={tab.title}>
                                                    {tab.title}
                                                </span>
                                            </div>
