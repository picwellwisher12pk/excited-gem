...
export const AITestModal: React.FC<AITestModalProps> = ({
  ...
  const createNewSession = async () => {
    const ai =
      (window as any).ai ||
      ((window as any).chrome && (window as any).chrome.aiOriginTrial) ||
      (navigator as any).ai
  ...
  const checkNativeAI = async () => {
    try {
      ...
      const ai =
        window.ai ||
        (window.chrome && window.chrome.aiOriginTrial) ||
        navigator.ai
      ...
      if (!ai && !edgeLM && !edgeSummarizer && !edgeWriter && !edgeRewriter) {
        setHasAI(false)
        setAiDetails(
          'No supported AI API (window.ai, LanguageModel, Summarizer, etc.) defined in this browser.'
        )
        return
      }
      ...
      if (ai && ai.languageModel) {
        ...
        if (typeof ai.languageModel.capabilities === 'function') {
          ...
          if (capabilities.modelName)
            setModelName(`Gemini Nano (${capabilities.modelName})`)
          else if (capabilities.model)
            setModelName(`Gemini Nano (${capabilities.model})`)
  ...
  const tabOperations = {
    ...
    toggleMuteTab: (id: number, currentMuted: boolean) =>
      chrome.tabs.update(id, { muted: !currentMuted }),
    togglePinTab: (id: number, currentPinned: boolean) =>
      chrome.tabs.update(id, { pinned: !currentPinned }),
  ...
  const detectIntentAndFetchData = async (input: string, startIndex = 0) => {
    ...
    const findTargets = (query: string) => {
      ...
      if (query.includes('current window') || query.includes('this window'))
        return allTabs.filter((t) => t.windowId === currentWindow.id)
      if (
        query.includes('audible') ||
        query.includes('sound') ||
        query.includes('noisy')
      )
        return allTabs.filter((t) => t.audible)
    ...
    if (text.includes('mute') && !text.includes('unmute')) {
      ...
      const validTargets =
        targets.length > 0 ? targets : allTabs.filter((t) => t.audible)
      ...
      for (const t of targets)
        if (t.id) await chrome.tabs.update(t.id, { muted: false })
      if (targets.length > 0) {
    ...
    if (text.includes('pin') && !text.includes('unpin')) {
      ...
      const targets = findTargets(text)
      for (const t of targets)
        if (t.id) await chrome.tabs.update(t.id, { pinned: false })
      if (targets.length > 0) {
    ...
    context +=
      `Browser Context - Tab List Batch (Showing ${limitedTabs.length} of ${fetchList.length} total found):\n` +
      limitedTabs
        .map(
          (t, idx) =>
            `${startIndex + idx + 1}. [${t.pinned ? 'PINNED' : 'TAB'}] ${t.title} (Status: ${t.audible ? 'AUDIBLE' : 'SILENT'}, ${t.mutedInfo?.muted ? 'MUTED' : 'UNMUTED'})`
        )
        .join('\n') +
    ...
    status = status
      ? `${status} + tab list`
      : `Listed ${limitedTabs.length} tabs`
  ...
  const cleanResponse = (text: string) => {
    ...
    const cleanedLines = lines.filter(
      (line) => !/^\d+\.\s.*\[(PINNED|TAB)\]/.test(line.trim())
    )
  ...
  const handleTestPrompt = async (
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () =>
          reject(
            new Error(
              'AI Request Timed Out (60s). The model is taking a while to process or initialize.'
            )
          ),
        60000
      )
    ...
    try {
      ...
      const {
        ...
        tabs,
        totalFound
      } = await detectIntentAndFetchData(userMessage, forcedStartIndex || 0)
      const fullPrompt = injectedContext
        ? `${injectedContext}User Question: ${userMessage}`
        : userMessage
  ...
  return (
    <Drawer
      ...
      footer={
        <div>
          ...
          {contextStatus && (
            <div
              style={{ fontSize: '11px', color: '#1890ff', marginBottom: 8 }}
            >
              ℹ️ {contextStatus}
            </div>
          )}
    ...
    >
      ...
      >
        {messages.length === 0 ? (
          <div
            style={{ textAlign: 'center', marginTop: 40, padding: '0 20px' }}
          >
            <Avatar
              icon={<Bot />}
              size={64}
            ...
            <Paragraph type="secondary">
              I'm your local AI assistant. I can see your open tabs and help you
              organize your work. Try asking <b>"What tabs do I have open?"</b>
            </Paragraph>
            ...
            {!hasAI && (
              <Alert
                ...
                description={
                  aiDetails ||
                  'Please check if Prompt API is enabled in your browser flags.'
                }
                type="warning"
                showIcon
              />
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-blue-500">
                        {msg.tabs.length} items
                      </span>
                    </div>
                    <div
                      className="ai-tab-list"
                      style={{
                        display: 'flex',
                        flexDirection: 'column',
                        gap: 0,
                    ...
                    >
                      ...
                      {msg.tabs.map((tab: any) => (
                        <div
                          key={tab.id}
                          className="relative rounded-lg overflow-hidden mb-1 border border-gray-100 hover:border-blue-200 bg-gray-50/30"
                        >
                          <Tab
                            {...tab}
                            activeTab={true}
                          ...
                          onClick={() =>
                            handleTestPrompt(
                              ...
                              msg.tabs
                                ? messages.filter(
                                    (m) =>
                                      m.role === 'assistant' && (m as any).tabs
                                  ).length * 10
                                : 10
                            )
                          }
